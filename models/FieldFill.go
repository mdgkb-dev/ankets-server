...
type FieldFill struct {
	bun.BaseModel `bun:"FieldFills,alias:FieldFills"`
	...
	// FieldVariantID uuid.NullUUID    `bun:"type:uuid" json:"FieldVariantId"`

	// SelectedFieldFillVariants          SelectedFieldFillVariants `bun:"rel:has-many" json:"selectedFieldFillVariants"`
	// SelectedFieldFillVariantsForDelete []uuid.UUID            `bun:"-" json:"selectedFieldFillVariantsForDelete"`
...
func (item *FieldFill) GetData(q *Field) interface{} {
	...
	if q.ValueType.IsSet() {
		res := ""
		// for _, v := range item.SelectedFieldFillVariants {
...
func (item *FieldFill) FieldFillVariantSelected(variantID uuid.NullUUID) string {
	res := No
	// for _, selectedVariant := range item.SelectedFieldFillVariants {
